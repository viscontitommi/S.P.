...
def tsrv_estimator(prices, K): #K=300 is 5 minutes
    ...
    # this estimate contains both real volatility and 2*n*E[noise^2]
    ...
    constraint_factor = 1 / (1 - (n_bar / n)) # Correction for small samples, at the beginning of formula
    
    tsrv = constraint_factor * (rv_avg - (n_bar / n) * rv_all) # formula for Zhang Estimator
...
def calculate_expected_noise_error(n, noise_std):
    """
    Calculates the expected bias in Realized Volatility due to microstructure noise.
    """
    noise_variance = np.array(noise_std ** 2)
    expected_bias = np.array(2 * n * noise_variance)
    return expected_bias


def calculate_expected_discretization_error(n, noise_std):
    """
    Calculates the expected magnitude (standard deviation) of the stochastic error 
    term O_p(n^(1/2)) for Gaussian noise.
    """
    noise_variance = np.array(noise_std ** 2)
    fourth_moment = np.array(3 * (noise_variance ** 2)) # For Gaussian noise: E[epsilon^4] = 3 * (variance^2) = 3 * noise_std^4
    estimator_variance = np.array(4 * n * fourth_moment) # Variance of the RV estimator due to noise is approx 4 * n * E[epsilon^4]
    stochastic_error_magnitude = np.sqrt(estimator_variance) #std. dev
    return stochastic_error_magnitude

def plot_theoretical_errors(noise_std, max_n=20000):
    n_values = np.arange(100, max_n, 100)  # Generiamo un range di frequenze n
    noise_bias = calculate_expected_noise_error(n_values, noise_std)
    discretization_error = calculate_expected_discretization_error(n_values, noise_std)

    plt.figure(figsize=(10, 6))
    plt.plot(n_values, noise_bias, 'r-', linewidth=2, label=r'Noise Bias ($2n\mathbb{E}[\epsilon^2]$)')
    plt.plot(n_values, discretization_error, 'b--', linewidth=2, label=r'Discretization Error ($O_p(\sqrt{n})$)')
    plt.title(f"Theoretical COmparison of Errors (Noise Std = {noise_std})")
    plt.xlabel("Sampling frequency ($n$)")
    plt.ylabel("Magnitude of Error (Variance)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.show()